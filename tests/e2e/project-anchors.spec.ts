...
test("app header and hero can target auto-generated anchors", async ({ page }) => {
  ...
  const menuSection = appHeaderEditorSheet
    ...
    .first();
  await menuSection.getByLabel("Label").fill("Features");
  await menuSection.getByLabel("Anchor id").click();
  await page.getByRole("option", { name: /\(features-1\)/ }).first().click();
  await menuSection.getByRole("button", { name: "Add item" }).click();
  await closeBlockEditor(page);

  await page.getByRole("button", { name: /Edit block .*Hero/i }).first().click();
  const heroEditorSheet = page.getByRole("dialog", { name: "Block editor" });
  await heroEditorSheet.getByLabel("Hero button anchor").click();
  await page.getByRole("option", { name: /\(features-1\)/ }).first().click();
  await closeBlockEditor(page);
